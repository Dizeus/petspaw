...
const Index = () => {
	...
	const {history, image} = useTypedSelector(state => state.vote)
						...
						{history.length>0?
							history.map(x=>
								<div key={x.id} className={styles.history__element}>
									<div className={styles.history__date}>{x.created_at.slice(11,16)}</div>
									<div className={styles.history__message}>Image ID: <span>{x.image_id}</span> was added to {x.value===1?"Likes":"Dislikes"}</div>
									<Image src={x.value===1?like:dislike} alt="icon" />
								</div>
								):
							"There is no history"	
							}
